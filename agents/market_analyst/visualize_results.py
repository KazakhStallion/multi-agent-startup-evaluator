...
EVAL_DIR = Path(__file__).parents[2] / "data" / "market_analyst" / "evaluation"
...
def plot_evidence_grounding(metrics):
    grounding = metrics.get("evidence_grounding_rate", {})
    if not grounding:
        print("no evidence grounding data, skipping that figure")
        return

    models = list(grounding.keys())
    rates = [grounding[m] for m in models]

    fig, ax = plt.subplots(figsize=(8, 5))
    bars = ax.bar(models, rates, color=sns.color_palette("muted", len(models)))
    ax.set_title("Evidence Grounding Rate per Model")
    ax.set_xlabel("Model")
    ax.set_ylabel("% of Outputs That Cited Real Dataset Numbers")
    ax.set_ylim(0, 115)
...
def plot_model_agreement(metrics):
    ...
    ax.set_title("Inter-model Agreement Rate (%)\n% of pitches where two models scored within 1 point")
    ax.set_xlabel("Model")
    ax.set_ylabel("Model")
    fig.tight_layout()
...
if __name__ == "__main__":
    ...
    plot_evidence_grounding(metrics)
    plot_rubric_scores(metrics)
    plot_risk_flagging(df)
